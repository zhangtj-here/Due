	...
	if (vnode.nodeType == 3) {
		...
		if (templates) {
			...
			for (let i = 0;i < templates.length; i++) {
				let templateValue = getTemplateValue([vm._data, vnode.env], templates[i]) //当前节点的对象，可以来自于Due对象，也可以来自于父级节点
...
function setTemplate2Vnode(template, vnode) {
	...
	let templateName = getTemplateName(template) // 获取字符串模板里面的值{{value}}
